...
FriendlyChat.prototype.prepareLoad = function() {
  ...
  var Gref = firebase.database().ref("Platform/InstantMessage/Messages/" + type);
...
FriendlyChat.prototype.loadMessages = function(Ref) {
  ...
  if (id != null) {
    this.messagesRef = firebase.database().ref('Platform/InstantMessage/Messages/' + type + "/" + ref + '/Message');
...
FriendlyChat.prototype.loadNameandUser = function(id) {
  ...
  function setName(Name) {
    //Load Name
    var names = firebase.database().ref("Platform/InstantMessage/Details/" + type + "/" + Name);
    ...
    //Load Members
    var members = firebase.database().ref("Platform/InstantMessage/Details/" + type + "/" + Name + "/Members");
  ...
  var ref = firebase.database().ref("Platform/InstantMessage/Details/" + type + "/" + idr + "/Role");
...
FriendlyChat.prototype.ListGroup2 = function() {
  var makeRef = firebase.database().ref("Platform/InstantMessage/Messages/Group");
  makeRef.once("value", function(Refdata) {
    Refdata.forEach(function (RefChildData) {
      Ref(RefChildData.val().Name, RefChildData.key);
      //.Name for Messages/Group Name||Details key  //Key for Messages/Group key
    });
  });

  function Ref(Name, id) {
  var Nameref = firebase.database().ref("Platform/InstantMessage/Details/Group/"+Name+'/Members');
  Nameref.once("value", function(NameChild) {
    NameChild.forEach(function(childData) {
      if (childData.val().email == firebase.auth().currentUser.email) {
      // console.log(childData.val().email);
      // console.log("Name");
      var ref = firebase.database().ref("Platform/InstantMessage/Details/Group/"+Name);
      ref.once("value").then(function(GroupData) {
...
FriendlyChat.prototype.ListEvent = function() {
  var ref = firebase.database().ref("Platform/InstantMessage/Details/Event");
...
FriendlyChat.prototype.AddTheMember = function() {
  ...
  var Gref = firebase.database().ref("Platform/InstantMessage/Details/"+type+"/"+id+'/Members');
...
FriendlyChat.prototype.saveMessage = function(e) {
  ...
  if (this.messageInput.value && this.checkSignedInWithMessage()) {
    ...
    var pushRef = this.database.ref('Platform/InstantMessage/Messages/' + type + "/" + MsgRef + '/Message');
