$(".content-container, .fit-container").parent().addClass("parent-container");

function enableLoading() {
    window.unloadedContent = $(".fit-container, .content-container").children().not("#header");
    unloadedContent.hide();

    var spinner = $('<div class="spinner">');
    spinner.append('<div class="double-bounce1">');
    spinner.append('<div class="double-bounce2">');

    $(".fit-container, .content-container").append(spinner);
}

function disableLoading() {
    $(".spinner").remove();
    window.unloadedContent.show();
}

$(window).on("scroll.scroll-navbar-detection", {
...
enableLoading();
$("#navbar-container").load("../html/nav-bar.html", function () {
    ...
    firebase.auth().onAuthStateChanged(function (user) {
        ...
        if (user != null) {
            ...
            firebase.database().ref("User").child(user.uid).once("value", function (data) {
                var profile = data.val();

                $('#user-identity').text(profile.fName + " " + profile.lName);
            });
            $('.login-controls').show();
        } else {
